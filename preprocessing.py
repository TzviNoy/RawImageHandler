...
def data_preprocessing(img: np.ndarray) -> (np.ndarray, np.ndarray):
    formatted_array = np.array([np.fromiter(format(element, '016b'), np.uint16) for element in img.flatten()])
